...
import { useCreateTagMutation } from '../../services/tagsService'
...
const CreateTagForm: FunctionComponent = (props) => {
    const steps = [
        { id: 0, title: "Create new tag", description: "Please fill in all required fields below!", isActive: true, isValidate: false }
    ]
    ...
    const handleCreate = async () => {
        try {
            validateRuls()
            if (isValidate) {
                const createTagData: any = await createTag({
                    name,
                    key
                }).unwrap()
    ...
    return (
        <Form action='post'>
            ...
            {stepState === 0 ?
                <>
                    {isCreate === true ? <Success text='Excellent. The tag has been successfully created.' /> : <></>}
